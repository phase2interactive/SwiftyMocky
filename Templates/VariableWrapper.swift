class VariableWrapper {
    ...
    var discardableAttribute: String {
    ...
    func propertyVerifyGetFactory() -> String {
        ...
        return "\(discardableAttribute)func \(variable.name)() -> VerifyBuilder<\(current.selfType), \(strippedType)> { VerifyBuilder(mock: mock, method: Verify(method: .\(propertyCaseGetName)), returning: (\(strippedType)).self, count: count, file: file, line: line) }"
    }

    func propertyVerifySetFactory() -> String {
        return "\(discardableAttribute)func \(variable.name)(set newValue: \(nestedType)) -> VerifyBuilder<\(current.selfType), Void> { VerifyBuilder(mock: mock, method: Verify(method: .\(propertyCaseSetName)(newValue)), returning: Void.self, count: count, file: file, line: line) }"
    }
    ...
    func givenFactoryName(prefix: String = "") -> String {
        ...
        return "\(attributes)func \(variable.name)() -> GivenBuilder<\(current.selfType), \(typeName)>"
    }
